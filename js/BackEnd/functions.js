...
(function($) {
	...
	$.fn.datetimePicker = function(options) {
		...
		return this.each(function(i) {
			...
			input.click(function(e) {
				...
				yearBlock.wheel(function(e) {
					if (e > 0)
					{
						yearBlock.find('a.arrow.down').mousedown();
						yearBlock.find('a.arrow.down').mouseup();
					}
					else
					{
						yearBlock.find('a.arrow.up').mousedown();
						yearBlock.find('a.arrow.up').mouseup();
					}
	...
	$.fn.wheel = function(callback) {
		return this.each(function() {
			...
			{
				if ('onwheel' in document) this.addEventListener("wheel", onWheel);
				else if ('onmousewheel' in document) this.addEventListener("mousewheel", onWheel);
				else this.addEventListener("MozMousePixelScroll", onWheel);
			}
			else
			{
